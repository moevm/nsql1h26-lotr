...
Если хотите мочь посмотреть содержимое БД через браузер, то пробросьте в `docker-co,pose.yml` в сервис `db` порты:
```bash
ports:
    - "127.0.0.1:7474:7474"
    - "127.0.0.1:7687:7687"
```
Помните, что это временная штучка, и не коммитьте это в репо.
