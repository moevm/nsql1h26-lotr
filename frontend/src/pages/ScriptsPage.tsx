  ...
  return (
    <>
      ...
      <FilterSection title="Name">
        {renderTextFilter('', 'name', 'e.g., tengwar')}
      </FilterSection>
...
const ScriptsPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useListScripts(queryParams);
  ...
  const adaptedData = data?.results?.map((script: any) => ({
    slug: script.slug,
    name: script.names?.[0] || 'Unknown',
    preview: [],
  })) || [];
  ...
  const handleAddClick = () => {
    ...
    if (user.role === 'admin') navigate('/create/script');
